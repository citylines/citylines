  ...
  def feature_popup_query(klass, ids)
    ...
    query = %Q{
      ...
      left join lateral (
        select
          #{aux_table_id},
          json_agg(json_build_object('name', lines.name,'url_name',lines.url_name, 'system', coalesce(systems.name,''),'historic',systems.historic,'project',systems.project,'transport_mode_name',transport_modes.name,'color',color,'from',fromyear,'to',toyear)) as lines
        from #{aux_table_name}
